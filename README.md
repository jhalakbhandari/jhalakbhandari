<h1 align="center">Hi 👋, I'm Jhalak Bhandari</h1>
<h3 align="center">Full-Stack Software Developer | MERN • .NET • Azure</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=jhalakbhandari&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

### 👩‍💻 About Me (10-second read)

* 💼 Software Developer at **Voiceback Analytics Pvt. Ltd.** (Product-based company)
* 🚀 1+ year experience building **scalable, production-ready web applications**
* 🧠 Strong in **MERN stack, .NET Core, REST APIs, SQL, Cloud & CI/CD**
* 🌱 Currently improving **DSA & problem-solving**
* ☁️ Hands-on with **Azure, Docker, GitHub Actions**

---

### 🛠️ Tech Stack

**Frontend:** React, Angular, HTML, CSS, Tailwind, Bootstrap
**Backend:** Node.js, Express, ASP.NET Core, .NET 6+
**Databases:** MongoDB, SQL Server
**Tools & Cloud:** Azure, Docker, Git, GitHub, CI/CD

---

### 📌 Projects

* 🔹 **Elite Essentials** – Product recommendation platform (React, Node, Prisma, Azure)
* 🔹 **Zaic** – B2B price benchmarking app (Angular, .NET, SQL Server)

---

### 🌐 Links

* 💼 Portfolio: [https://jhalakbhandari.netlify.app/](https://jhalakbhandari.netlify.app/)
* 📫 Email: **[bhandari.jhalak2018@gmail.com](mailto:bhandari.jhalak2018@gmail.com)**
* 🔗 LinkedIn: [https://www.linkedin.com/in/jhalak-bhandari-3680961aa](https://www.linkedin.com/in/jhalak-bhandari-3680961aa)
* 🧩 LeetCode: [https://leetcode.com/jhalakbhandari](https://leetcode.com/jhalakbhandari)

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=jhalakbhandari&show_icons=true&theme=default" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=jhalakbhandari&layout=compact" height="150" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=jhalakbhandari" />
</p>

---

<p align="center">✨ Building clean systems. Shipping real products. Growing every day.</p>
